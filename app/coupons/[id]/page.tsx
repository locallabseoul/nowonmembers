...
import { CalendarDays, MapPin, Phone, ShieldCheck, Store, Ticket, Users } from "lucide-react";
...
const labels = { scheduled: "공개 예정", claiming: "쿠폰 받기", claim_closed: "발급 마감", expired: "사용 종료" } as const;
      ...
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
        <section className="overflow-hidden rounded-[24px] border border-slate-100 bg-white shadow-sm">
          ...
          <div className="p-6 sm:p-8">
            <p className="text-lg font-black text-primary">{getCouponBenefitLabel(coupon)}</p>
            <h1 className="mt-2 text-3xl font-black text-charcoal">{coupon.title}</h1>
        ...
        <aside className="h-fit rounded-[24px] border border-slate-100 bg-white p-6 shadow-sm lg:sticky lg:top-24">
          ...
          <div className="mt-5 space-y-4 text-sm text-gray-600">
            <p className="flex gap-3"><Ticket size={17} className="shrink-0 text-primary" />발급 {coupon.claimStart} ~ {coupon.claimEnd}</p>
            <p className="flex gap-3"><CalendarDays size={17} className="shrink-0 text-primary" />사용 {coupon.useStart} ~ {coupon.useEnd}</p>
          ...
          ) : <button disabled className="mt-6 w-full rounded-xl bg-gray-100 py-4 text-sm font-black text-gray-400">{labels[status as keyof typeof labels] ?? "발급 불가"}</button>}
