...
type WalletRow = {
  ...
  next_bonus_expiration_at: string | null;
};
...
export async function getBusinessPointOverview(): Promise<BusinessPointOverview> {
  ...
  const [{ data: ledgerRows }, { data: orderRows }, { data: reservationRows }] = await Promise.all([
    ...
    supabase
      ...
      .select("id,order_id,point_amount,bonus_points,total_amount,refunded_points,status,payment_key,paid_at,created_at,point_lots(kind,available_points,reserved_points)")
      .eq("business_id", walletRow.business_id)
  ...
  const orders = (orderRows ?? []).map((row) => {
    ...
    return {
      ...
      paidAt: row.paid_at ?? "",
      createdAt: row.created_at,
      refundablePoints: lots.reduce(
        (sum, lot) => sum + (lot.kind === "paid" ? Number(lot.available_points ?? 0) : 0),
        0
      )
    };
  });
  ...
  return {
    wallet: {
      ...
      bonusPoints: Number(walletRow.bonus_points),
      nextExpirationAt: walletRow.next_expiration_at,
      nextBonusExpirationAt: walletRow.next_bonus_expiration_at
    },
    ledger,
    orders,
